...
**Record which skills each subagent actually invoked — from its transcript, not its words.** Each `Task` call returns that subagent's transcript path (its `output_file`, a `.jsonl` or `.output` under the session `tasks/` dir) — **capture that exact path verbatim when it is returned**; a `SendMessage` reply continues the same agent and appends to the same file. **Grep it once the agent has finished with the issue — not after every reply.** The transcript is flushed when the agent's lifecycle ends, so grepping right after a `Task`/`SendMessage` reply reads a 0-byte file and looks exactly like "the agent skipped its mandated skills" (this produced a false process-miss report on issues #1–#2, and the same files later measured 1.2 MB and 2.0 MB with every skill present). Grep when you write the issue's audit log in Step 4b, after the last reviewer reply for that issue: `grep -o '"skill":"[a-z:_-]*"' <output_file> | sort | uniq -c` and `grep -c '"name":"Skill"' <output_file>`. If the file is still empty then, record that the transcript was unavailable — never record "0 skills invoked". Verify BOTH the four pipeline skills AND every domain skill the reviewer mandated in the delegation prompt (e.g. `antd-ui`, `frontend-design`, `dataviz`); a mandated domain skill absent from the implementer's transcript is a process miss to record and, if outcome-relevant, feed back as a fix pass. Record the verified result into the per-issue Thai audit log (Step 4b below). The final batch report must be **in Thai (ภาษาไทย)**.
