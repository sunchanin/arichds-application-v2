...
def validate_capture_dir_setting(path_str: str, allowlist: list[Path]) -> Path:
    ...
    if ".." in Path(path_str).parts:
        raise ValueError("capture_dir must not contain '..' components")

    candidate = Path(path_str)
    if not candidate.is_absolute():
        raise ValueError(f"capture_dir must be an absolute path, got {path_str!r}")
    ...
    if final_stat is not None:
        if stat.S_ISLNK(final_stat.st_mode):
            raise ValueError(f"capture_dir must not be a symlink, got {path_str!r}")
        if not stat.S_ISDIR(final_stat.st_mode):
            ...
            raise ValueError(f"capture_dir already exists and is not a directory: {path_str!r}")
    ...
    if resolved.parent == resolved:
        raise ValueError(f"capture_dir must not be a filesystem or drive root, got {resolved!r}")

    if allowlist and not any(resolved == root or resolved.is_relative_to(root) for root in allowlist):
        raise ValueError(f"capture_dir {path_str!r} is not within the permitted capture allowlist")
