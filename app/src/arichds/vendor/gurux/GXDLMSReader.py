...
class GXDLMSReader:
        ...
        try:
            if self.client.negotiatedConformance & Conformance.ACCESS != 0:
                for it in objs:
                    ...
                    elif isinstance(it, (GXDLMSDemandRegister)) and it.canRead(4):
    ...
    def getReadOut(self):
        ...
        for it in self.client.objects:
            ...
            self.writeTrace(
                "-------- Reading " + str(it.objectType) + " " + str(it.name) + " " + it.description,
                TraceLevel.INFO,
            )
    ...
    def __getPath(cls, securitySuite, type_, path, systemTitle):
        ...
        return os.path.join(path, pre + GXDLMSTranslator.ToHex(systemTitle, False) + ".pem")
    ...
    def generateCertificates(self, logicalName):
        ...
        certs = []
        if self.client.authentication < Authentication.LOW:
            raise Exception("High authentication must be used to change the certificate keys.")
        reply = GXReplyData()
        ...
        key = GXPkcs8(kp)
        key.save(GXPkcs8.getFilePath(Ecc.P256, CertificateType.DIGITAL_SIGNATURE, clientST))
        # Generate x509 certificates.
        pkc10 = GXPkcs10.createCertificateSigningRequest(kp, subject)

        # All certigicates are generated with one request.
        certifications.append(GXCertificateRequest(CertificateType.DIGITAL_SIGNATURE, pkc10))
        ...
        pkc10 = GXPkcs10.createCertificateSigningRequest(kp, subject)
        # All certigicates are generated with one request.
        certifications.append(GXCertificateRequest(CertificateType.KEY_AGREEMENT, pkc10))

        # Generate public/private key for digital signature.
        if not self.readDataBlock(ss.generateKeyPair(self.client, CertificateType.DIGITAL_SIGNATURE), reply):
            raise GXDLMSException(reply.error)

        reply.clear()
        # Generate public/private key for key agreement.
        if not self.readDataBlock(ss.generateKeyPair(self.client, CertificateType.KEY_AGREEMENT), reply):
            raise GXDLMSException(reply.error)
        reply.clear()
        ...
        if not self.readDataBlock(ss.generateCertificate(self.client, CertificateType.KEY_AGREEMENT), reply):
            raise GXDLMSException(reply.error)
        ...
        for it in certs:
            if it.subject.find(GXAsn1Converter.systemTitleToSubject(ss.serverSystemTitle)) != -1:
        ...
        for it in certs:
            if not self.readDataBlock(
                ss.exportCertificateBySerial(self.client, it.serialNumber, it.issuerRaw),
                reply,
            ):
                raise GXDLMSException(reply.error)
            # Verify certificate.
