...
class Prometer100Driver(DlmsProfileDriver):
    ...
    # No battery capability (ADR 0011 — a flag turns on from a meter). The CEWE
    # battery-status register ``0.0.96.6.1.255`` answers "Device reports a
    # undefined object" on both Prometer 100 units we can reach — WP079074 and
    # WP080652, probed 2026-09-16 (``docs/meter-notes/cewe-battery-scan.md``,
    # ui-audit ticket 04). The only member of the ``0.0.96.6.x`` group that
    # answers is ``E=0``, a battery use-time counter, which is not a status.
    # The base class's ``supports_battery()`` (False) and its raising
    # ``read_battery_status()`` therefore stand; M7-2's override is gone.
