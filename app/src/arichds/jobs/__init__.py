...
(the load-profile cycle is in :mod:`arichds.acquisition.load_profile`, and M5c's
retention and backup jobs in :mod:`arichds.db.retention` and
:mod:`arichds.db.backup` — their domain is the rows and the file, not a meter);
only the registry entry lives here. That is what makes adding M6's billing
auto-read, or M8's sync, one entry in a list plus a function — never a new
