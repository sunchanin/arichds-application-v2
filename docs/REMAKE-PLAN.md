...
**M4c (หลัง M6 จบ) — รุ่นที่เหลือทั้งหมด** *(แตกเป็นสองเฟสที่ grill M4c, 2026-08-09 —
รายละเอียดที่เคาะแล้วทั้ง 14 ข้ออยู่ใน `SPEC.md` §3.4/§3.5/§3.6)*

**เส้นแบ่งคือ "ยิงมิเตอร์ถึงหรือไม่" ไม่ใช่ยี่ห้อ** — เพราะ exit ของเฟสนี้คือ parity ซึ่งต้องมี
มิเตอร์จริงและต้องมี v1 ที่รันได้ (เจ้าของยืนยัน 2026-08-09 ว่ามีเครื่องที่ยก v1 ขึ้นมาได้):

| เฟส | รุ่น | Exit |
|---|---|---|
| **c1** | **CEWE ครบทั้งสามรุ่น** — Prometer 100 `203.170.151.152:4059` · Saral 305 `203.170.151.217:4059` · Premier 550 `49.229.159.44:**50001**` | **เทียบสด v1 vs v2 ที่ระดับแถวในฐาน** แล้วแช่ buffer ดิบเป็น replay fixture |
| **c2** | **SMART TCC ×5** `203.170.148.103` — **ยิงไม่ถึง** | เขียน driver + เทสด้วย fixture จากสแกน · **exit ค้างจนกว่าจะได้ลิงก์** |

**เคส 2 logger พิสูจน์ครบทั้งสองครึ่งใน c1** (ทดสอบ TCP 2026-08-09 — เปิดทั้งสามเครื่อง) ·
**Prometer 100 = ครึ่งเชิงคาบ** (900 กับ 300 วิ ⇒ ครบวัน 96 กับ 288 คนละเลข แต่ Logger 2 ของมัน
มีค่าที่ตรงกับคอลัมน์เราแค่ความถี่ ⇒ พิสูจน์ *ท่อ* ไม่ใช่ *ค่า*) · **Premier 550 = ครึ่งเชิงค่า**
(Logger 1 มีแต่พลังงาน · V L-N ×3 และ I ×3 อยู่ใน Logger 2 ทั้งหมด ⇒ เป็นรุ่นเดียวที่ทำให้ทั้งการ
แยกแถวและการรวม logger ตอนส่งออก CSV มีความหมาย) · ส่วนเคส "คอลัมน์ชนกัน" ของ Premier 550
ปิดไปแล้วด้วยการ **ไม่ map พลังงานสะสม `x.8` ของ Logger 2** (SPEC §3.5)

**โครง 2 ใบ** (เคาะหลัง scrutinize รอบสอง 2026-08-09):

| ใบ | เนื้องาน |
|---|---|
| **#24 — CEWE ครบสามรุ่น** | watermark แยกต่อ `(device, logger)` · driver Prometer 100 / Saral 305 / Premier 550 **ทั้ง LP และ billing พร้อมกัน** · billing +20 คอลัมน์ · กุญแจ `(OBIS, attr)` · profile OBIS ต่อรุ่น · ตัวจำแนกรอบเปิด · API 12 → 63 ฟิลด์ · หน้า Billing 60 คอลัมน์ · capture +4 หมวด |
| **#25 — SMART TCC ×5** | driver + เทสด้วย fixture จากสแกน · **ค้างที่ลิงก์ ไม่ใช่ที่โค้ด** · blocked by #24 |

> **รอยตัดที่มีอะไรบังคับมีรอยเดียว: ยิงมิเตอร์ถึง กับ ยิงไม่ถึง** — ร่างก่อนหน้านี้แตกเป็น 5 ใบ
> โดยอ้างเพดาน "~19 ไฟล์ต่อใบ" ซึ่ง**ขัดกับคอมมิตของโปรเจกต์นี้เอง**: `c27ba96` (#21) แตะ **32 ไฟล์**
> และ `e8fef84` (#22) แตะ **45 ไฟล์** — ทั้งคู่ข้าม `app/` + `web/` ในใบเดียว ผ่านด้วยรีวิว 2 รอบ
> ไม่มีข้อบกพร่องหลุดถึง commit · ใบ 1 ประเมินได้ ~30-34 ไฟล์ จึงเล็กกว่าใบที่ผ่านมาแล้ว
>
> และการแยก LP ออกจาก billing **เพิ่มงานจริง ไม่ใช่แค่เพิ่มใบ**: ไดรเวอร์แต่ละตัวต้องมีทั้งสอง
> ความสามารถ ⇒ `saral305.py`/`premier550.py` จะถูกสร้างในใบหนึ่งแล้วเปิดแก้อีกใบ และ `smw110.py`
> จะโดนแก้สองครั้ง (ลายเซ็นต่อ logger · กุญแจ `(obis, attr)`)

**การปลดล็อกรุ่นใหม่ = เพิ่มบรรทัดใน `drivers/factory.py` เท่านั้น** — `catalog.py:10-12`
มีครบทั้ง 9 รุ่นอยู่แล้วและคำนวณ `driver_status` จาก `factory.supported_models()` เอง
(ร่างก่อนหน้านี้เขียนว่าต้องแตะ `catalog.py` ด้วย — ผิด)

**parity วัดที่แถวในฐาน ไม่ใช่ที่หน้าจอ** (CONTEXT.md — Output Parity) · เหตุผลที่บังคับ:
หน้า Load Profile ของ v1 ฮาร์ดโค้ด `DEFAULT_LOGGER_ID = 1` ทั้งสามจุด ⇒ **v1 ไม่มีหน้าจอไหน
แสดง Logger 2 เลย** ที่เดียวที่มันโผล่ถึงลูกค้าคือ CSV ซึ่ง v1 รวมสอง logger เป็นแถวเดียว
⇒ ถ้าวัดที่หน้าจอ เคส 2 logger จะวัดไม่ได้ · **parity run เป็นงานของเจ้าของ ไม่ใช่ใบ issue**
...
> ไม่ใช่ connection-refused** ⇒ เครื่องไม่ตอบอะไรเลย เป็นเรื่องเส้นทาง/ไฟร์วอลล์ ไม่ใช่ "พอร์ตปิด"
> ซึ่งแปลว่าแก้ที่ฝั่งเราไม่ได้ · และไฟล์สแกนนั้นครอบ **รุ่น test 3CL รุ่นเดียวจากห้ารุ่น** (v1 ใช้
> `smart_tcc.py` ตัวเดียวคุมทั้งห้า แต่ v2 ยังไม่มีหลักฐานว่าอีกสี่รุ่นเหมือนกันจริง)
> ⇒ **5 จาก 9 รุ่นค้างอยู่ที่ข้อนี้ข้อเดียว** — ต้องตามเรื่องลิงก์ขนานไปกับ c1 ไม่ใช่รอถึงตอนเริ่ม c2
...
> `.scratch/` สามโฟลเดอร์ · **ตกลงครบ 19 ข้อ ไม่มีคำถามค้างกับลูกค้าแล้ว** · โมดูลแรก
> (Billing) ผ่าน `/grill-with-docs` เมื่อ 2026-09-09 — §7.3
>
...
| # | เรื่อง | ผลสรุป |
|---|---|---|
| A1·A2 | LP เพิ่ม 11 คอลัมน์ | ตกลง · แยกเป็นครึ่งที่ 3 ตระกูลมี (มุมเฟส + Record Status) กับครึ่งที่ Prometer 100 มีตัวเดียว |
| A3 | billing CSV แบบต่อท้าย | ตกลง · **ของใหม่ทั้งก้อน — ทั้ง v1 และ v2 ไม่เคยมีไฟล์นี้** (v1 ออกเป็น PDF/xlsx รายรอบ) |
| A4 | ชื่อหัวคอลัมน์ | ใช้ของ v2 ที่แก้หน่วยแล้ว (`Import Reactive (kvarh)`) แล้ว**แจ้ง**ลูกค้า ไม่ใช่ถาม |
| B1 | เห็นมิเตอร์ที่ดึงไม่สำเร็จ | **ช่องว่างการแสดงผล ไม่ใช่ข้อมูล** — `consecutive_failures` มีมาตั้งแต่ ADR 0004 แต่ไม่มีหน้าไหนแสดง |
...
| D1 | บันทึก Energy Summary | **ไฟล์ ไม่ใช่ database** ⇒ ADR 0012 ไม่ถูกแตะ |
...
| E3 | Special Days → database | ตาราง `Holiday` มีอยู่แล้ว · **invariant ไม่ถูกท้าทาย** งานคือทางนำเข้า |
| E4 | ปลายทาง API | **push ออก** ⇒ คือ M8 เดิม ไม่ใช่งานใหม่ |
| E5 | 7 หน้าที่ส่งรูปมาแต่ไม่มีโน้ต | ยังไม่ต้องทำ |

> ⚠️ **E1 — ทิศทางของ feature key ต้องกลับด้าน** · `constants.py` เตือนไว้เองว่า license ที่เซ็น
> ด้วยรายการ feature **แบบระบุชัด** จะไม่ได้ key ที่เพิ่มทีหลัง *"เงียบๆ โดยไม่มีคำเตือนที่ไหนเลย"*
> — และ license ทั้งสองใบที่ออกให้ลูกค้าเซ็นแบบระบุชัด ⇒ **key ต้องแปลว่า "เพิ่มมิเตอร์ได้โดย
> ไม่ต้องมี code"** เพื่อให้ "ไม่มี key = พฤติกรรมเดิม" · ถ้าทำกลับด้านเป็น "บังคับให้กรอก"
> **จะปลดล็อกเครื่องที่ส่งไปแล้วทุกเครื่อง** · จุดที่ด่านอยู่คือ `api/devices.py:1003` แต่ฟีเจอร์
> แตะอย่างน้อย 6 ที่ (`constants.py` · ด่าน · ฟอร์ม Add-device · `web/src/features.ts` ·
> `tools/arichds_vendor.py` · เทส)

### 7.3 M11 (เสนอ) — Billing: All-Meters View
...
|---|---|---|
| 1 | **M11 Billing All-Meters View** | grill เสร็จแล้ว · ไม่แตะ ADR ไหน ไม่รอฮาร์ดแวร์ ไม่รอลูกค้า |
| 2 | **M12 Licensing (E1)** | เล็ก ชัด แต่ต้อง grill เองเพราะกระทบทุก license ที่จะออกต่อจากนี้ |
| 3 | **M13 ไฟล์ export** | เรียง **formatter ก่อน driver** — billing CSV → Energy file → คอลัมน์ที่ขาด · สองก้อนแรกไม่พึ่งก้อนหลังเลย ลูกค้าได้ของใช้เร็วกว่ามาก |
| 4 | **M14 E3 → E2** | E3 ตรงไปตรงมาแล้ว · E2 ต้องมี ADR ก่อนลงมือ |
| 5 | **build + ส่งมอบ** | เจ้าของสั่งให้ build ทีเดียวหลัง implement ครบ |
...
### 7.5 บั๊กที่เจอระหว่างตรวจ — `avg_geo_pf` ว่างทุกแถว
...
ยังไม่ไล่สาเหตุ · ผู้ต้องสงสัยสามทาง — scaler/unit path ของ `Unit.NONE` · การ match key
`(OBIS, attribute)` · มิเตอร์คืน null · **ต้องมี issue ของตัวเองและใช้ `/diagnosing-bugs`
อย่าเดาลงไปใน issue body**
