...
token. Fully implemented: M14 ticket 03 landed the Push Token issue/verify primitive, ticket 07
landed the configuration, the admin endpoints and the published contract, and ticket 08 landed
the push cycle itself — the `central_push` scheduler job, the stdlib-`urllib` holdings/push HTTP
client, and everything that writes `centralpush/status.py` (see CLAUDE.md's digest for all
three).
